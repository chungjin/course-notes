...
### 转化为01背包问题
其实原理和iterate, k in [0, M[i]]差不多。为了加快计算，把物品分成好几堆，每次要么全部取，要么不取。于是degrade到0-1背包问题。
...
因为`1，2，4，6`这4个数，已经可以完全表示0-13之间的所有数字。(参考二进制的表示方法）

```python
# 背包当前容量V, 第i件物品重量为C, 价值为W, 数量有M
def MultiplePack(F, C, W, M, V)
    # 背包可以放得进全部M件物品
    if C · M <= V
        CompletePack(F,C,W)
        return
